...
import { toUint8Array, TrustAllianceAgent, transformParams, uriToHex } from "../../src/agent";
...
import { TrustAllianceModules, TrustAllianceCallables } from "../../src/agent/enums";
...
const savedDIDStructure = {
  ...
  did_ref: "https://ipfs.trustalliance.dev:8080/ipfs/Qma3fM3VBKPXt7peEeJCAG25s7QMUwvLvGDwFikonZbPff",
...
const didDocument = {
  ...
  id: "did:trustalliance.dev:0xaseb1b3",
  assertionMethod: [
    {
      id: "did:trustalliance.dev:dia-0x12345678999",
...
const didRef = "https://ipfs.trustalliance.dev:8080/ipfs/did-document-cid";
...
describe('DID operation tests', () => {
  ...
  it("Should call dispatch once when creating a DID Document", async () => {
      ...
      assert(
        stub.calledOnceWith(
            account, 
            TrustAllianceModules.DIDModule,
            TrustAllianceCallables.DIDInsert,
            transformed
        )
      );
  ...
  it("Should call dispatch once when updating a DID Document", async () => {
    ...
    assert(
      stub.calledOnceWith(
          account, 
          TrustAllianceModules.DIDModule,
          TrustAllianceCallables.DIDUpdate,
          transformed
      )
    );
  ...
  it("Should revoke a DID Document", async () => {
    ...
    assert(
      stub.calledOnceWith(
          account, 
          TrustAllianceModules.DIDModule,
          TrustAllianceCallables.DIDRevoke,
          transformed
      )
    );
  });
  ...
  it.skip("Should fetch the DID Document from CID", async () => {
    await cryptoWaitReady().then(async () => {
      agent = new TrustAllianceAgent(new Connector());
