...
# TrustAlliance Agent SDK

* Create, Revoke, Resolve, Update DIDs
* Connects with TrustAlliance chain
...
nvm use 14
npm install @trustalliance/agent
```
...
<details>
  ...
  ```javascript
import {CredentialIssuer,Connector, createAccount, TrustAllianceAgent} from '@trustalliance/agent';

async function createAnIssuer() {
    // Create a Connector object.
    const connector = new Connector();

    // Initialise the `TrustAllianceAgent` with a connector instance.
    const agent = new TrustAllianceAgent(connector);
    
    // An Issuer needs an active account on TrustAlliance Blockchain
     ...
     * {
            ...
            did_ref: 'https://ipfs.trustalliance.dev:8080/ipfs/QmdH3JyxJ1A45goKAt18ja3TX67ea4Vz2NTyLkLXEQYTk8',
            ...
            did_document: {
                ...
                id: 'did:trustalliance:e97aaebd-c221-44d6-925f-f73493eec3dd',
            ...
            header: {
                alg: 'EdDSA',
                typ: 'JWT',
                kid: 'did:trustalliance:key:JsonWebKey2020:ZpM9EIlXv2Yj9F1IaIS-S1-BkSrXIq8ad2rpAffDDlg#ZpM9EIlXv2Yj9F1IaIS-S1-BkSrXIq8ad2rpAffDDlg'
            },
            payload: {
                nbf: 1635477651,
                iss: 'did:trustalliance:e97aaebd-c221-44d6-925f-f73493eec3dd',
                vp: {
                '@context': [Array],
                type: [Array],
...
async function createCredentials() {
    ...
    const connector = new Connector();
    // Creates the TrustAlliance Agent
    const agent = new TrustAllianceAgent(connector);
    
    // Creates the defualt account for the issuer
    // `Alice` if there's no mnemonic
    // The passing mnemonic must be a valid account on TrustAlliance
  ...
  async function verify(jwtPresentation) {
    ...
    // For the MVP please use an Account available on TrustAlliance Chain.
...
import { TrustAllianceAgent } from '@trustalliance/agent'
...
const { TrustAllianceAgent } = require('@trustalliance/agent');
...
*/
agent = new TrustAllianceAgent(new Connection());
...
*/
let agent = new TrustAllianceAgent(new Connector());
...
import { CredentialIssuer } from '@trustalliance/agent'
import { createAccount } from '@trustalliance/agent/account'
...
const { DefaultOptions } = require('@trustalliance/agent')
const options = {
  url: 'ws://custom.node.example.com', // custom node url ws[s]://custom.node.example.com[:9944]
  options: {...DefaultOptions.options} // using trustalliance defaults
}

const agent = new TrustAllianceAgent(new Connector(options));
...
const connector = new Connector();
const account = connector.createAccount({name:"TrustAlliance"});
...
// https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Falpha-node.trustalliance.dev#/accounts
