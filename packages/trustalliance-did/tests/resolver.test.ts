...
describe('DID resolver Tests', function () {
  describe('Tests for parseDID()', () => {
    it('parseDID should return trustalliance', () => {
      const did = new DIDResolver();
      const result = did.parseDID('did:trustalliance:1324');

      expect(result).to.be.an('object').to.have.property('prefix');

      expect(result.prefix).to.equal('trustalliance');
    });
  ...
  describe('Tests for resolve()', () => {
    ...
    it('should not resolve for unknown method', async () => {
      const { method, resolver } = mockOptions('trustalliance');
    ...
    it('should resolve for known method', async () => {
      ...
      const did = 'did:trustalliance:test1234';
